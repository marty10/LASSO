...
for d,dir_turbine in enumerate(np.array([-0.5,0.5,1,-1])):
    ...
    if d==0:
        ...
        X_transf = np.concatenate((X_transf, X_1), axis = 1)
...
while num_cycle<cycles:
    ...
    np.savez(output_folder+"/Enel_cross_val_blocks_direction_single_plus_verso.npz", dict_ = output_dict,saved_indexes_list = saved_indexes_list, mses = mses, weights_list = weights_list,
