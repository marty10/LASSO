...
print("feature dim single turbines: ", X_transf.shape[1])
X_transf, output_dict_ = enel_transf.transform1(turbine_dict, enel_dict, X, power_curve, 1, X_transf,
                                                    output_dict_)
print("feature dim mean turbines: ", X_transf.shape[1])
...
values_TM = []
#new_loss, _ = compute_lasso(XTrain_, YTrain_, XVal_, YVal_,score = "mean_squared_error", values_TM = values_TM)
#print("loss", new_loss)
...
while num_cycle<cycles:
    ...
    np.savez(output_folder+"/Enel_cross_val_blocks_direction_train.npz", dict_ = output_dict_,saved_indexes_list = saved_indexes_list, mses = mses, weights_list = weights_list,
