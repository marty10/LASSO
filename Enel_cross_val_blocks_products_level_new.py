...
while num_cycle<cycles:
    ...
    countIter+=1
    np.savez("Enel_cross_val_blocks_level_products_new"+str(k)+".npz", dict_ = dict_,saved_indexes_list = saved_indexes_list, mses = mses, weights_list = weights_list, XTrain = XTrain,
             XTest = XTest, YTest = YTest, YTrain = YTrain, XTrainTransf_ = XTrain_transf, XTestTransf_ = XTest_transf,
             XTrain_ValNoCenter = XTrain_noCenter, XValTransf_noCenter = XVal_noCenter, YTrainVal_noCenter = YTrain_noCenter,
             YVal_noCenter = YVal_noCenter, XTrain_Val = XTrain_, XVal = XVal, YVal_ = YVal_, YTrain_Val = YTrain_ )
