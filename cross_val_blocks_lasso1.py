...
from utility import generate_samples, get_current_data, extract_losses, get_common_indexes, \
    extracte_chosen_indexes, compute_mse, get_common_indexes1
...
while len(num_informative)<n_informative and len(saved_indexes)<active_set:
    ...
    for i in range(0, num_blocks):
        ...
        new_loss,beta = compute_mse(lasso, x_train_i, YTrain,x_test_i, YTest)
    ...
    weights_indexes = get_common_indexes1(orderd_losses_,ordered_loss_ten,blocks_generated,n_features,betas)
...
np.savez("cross_val_blocks_"+str(active_set)+"lasso.npz", saved_indexes = saved_indexes)