...
while len(saved_indexes)+chosen_indexes<=final_active_set-len(deleted_indexes):
    ...
    iter+=1
    np.savez("cross_val_blocks_"+str(num_blocks)+"active_set"+ str(final_active_set)+"dynamic_set_.npz", saved_indexes_list = saved_indexes_list, mses = mses, num_informative_list = num_informative_list, weights_list = weights_list)